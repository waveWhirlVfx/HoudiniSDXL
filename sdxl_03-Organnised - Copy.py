...
class SDXLPanel(QtWidgets.QWidget):
    ...
    def initUI(self):
        # Wrap UI in a scroll area for responsiveness
        ...
        # Only T2IAdapter-based control models are included now.
        control_models = [
    ...
    def generate_image(self):
        ...
        try:
            ...
            if not self.cancel_flag and result.images:
                ...
                # If "Generate Maps" is turned on, generate maps and create shader.
                if self.maps_checkbox.isChecked():
                    ...
                    self.update_status(f"Base image saved: {base_path} | Took {elapsed_time:.2f} seconds")
                self.history.append(f"{timestamp} - {base_path} - Seed: {self.seed_input.value()}")
                self.history_list.addItem(f"{timestamp} - {base_path} - Seed: {self.seed_input.value()}")
