...
# ----------------------- DeepCache pruna Optimization -----------------------
def pruna(pipe):
    """
    pruna applies DeepCache optimization to the given pipeline.
    
    This function is a placeholder for the actual DeepCache algorithm.
    ...
    For now, it logs the optimization step and returns the pipeline.
    """
    logger.debug("Applying DeepCache pruna optimization to the pipeline.")
    # Example: If there are specific modules in the pipe that can be optimized,
    # you could replace them with optimized versions here.
    # For instance:
    #   pipe.some_module = optimized_module(pipe.some_module)
    #
    # Currently, no modifications are made.
    return pipe

# ----------------------- Utility Function for Optimizing Pipeline -----------------------
def optimize_pipeline(pipe):
    """
    Applies a series of optimizations to the pipeline, including:
    - Memory efficient attention
    - CPU offloading
    - Attention slicing
    - VAE tiling and slicing
    - Forward chunking for the VAE
    - DeepCache pruna optimization
    """
    ...
    # Apply DeepCache pruna optimization
    pipe = pruna(pipe)
    return pipe
...
class SDXLPanel(QtWidgets.QWidget):
    ...
    def initUI(self):
        ...
        # Top Splitter: Prompt and Preview side by side
        ...
        # Control Models Group (Collapsible)
        ...
        # LoRA Settings Group (Collapsible)
        ...
        # Maps Preview Group (only shown if "Generate Maps" is checked)
    ...
    def get_pipeline_key(self):
        ...
        if self.ti_token.text():
            key += f"_ti_{self.ti_token.text()}"
        return key
    ...
    def generate_image(self):
        ...
        try:
            ...
            # Check for a cached pipeline if 'Keep models loaded in GPU' is enabled.
            ...
            if ti_enabled:
                self.status_signal.emit("Applying Textual Inversion...")
                pipe.load_textual_inversion(self.ti_token.text())
            ...
            # Update cache if 'Keep models loaded in GPU' is enabled.
            ...
            # Only add 'strength' if no control adapter is being used
