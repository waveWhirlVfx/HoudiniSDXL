...
class SDXLPanel(QtWidgets.QWidget):
    ...
    def initUI(self):
        ...
        # Top Splitter: Prompt inputs and Previews side by side
        ...
        # Right: Input/Output Preview Section
        ...
        # Maps Preview Group
        maps_group = QtWidgets.QGroupBox("Maps Preview")
        maps_layout = QtWidgets.QHBoxLayout(maps_group)
        ...
        gen_layout.addWidget(maps_group)

        # Control Buttons and Progress/Status Area
        ...
        # Add Generate Tab to Tab Widget
    ...
    def create_principled_shader(self, base_path, bump_path, specular_path, roughness_path, displacement_path):
        ...
        # Set file texture parameters (parameter names may vary by shader version)
        shader.parm("basecolor_useTexture").set(1)
        shader.parm("basecolor_texture").set(base_path)
        shader.parm("bump_useTexture").set(1)
        shader.parm("bump_texture").set(bump_path)
        shader.parm("specular_useTexture").set(1)
        shader.parm("specular_texture").set(specular_path)
        shader.parm("rough_useTexture").set(1)
        shader.parm("rough_texture").set(roughness_path)
        shader.parm("disp_useTexture").set(1)
        shader.parm("disp_texture").set(displacement_path)
        shader.moveToGoodPosition()
        self.update_status("Created Principled Shader 'generated_principled' in /mat")
        return shader
    ...
    def generate_image(self):
        ...
        try:
            # Model loading and pipeline initialization (simplified for brevity)
            ...
            if not self.cancel_flag and result.images:
                ...
                # Update map previews in the UI
                bump_pixmap = self.pil_image_to_pixmap(bump_map.convert("RGB"))
                specular_pixmap = self.pil_image_to_pixmap(specular_map.convert("RGB"))
                roughness_pixmap = self.pil_image_to_pixmap(roughness_map.convert("RGB"))
                displacement_pixmap = self.pil_image_to_pixmap(displacement_map.convert("RGB"))
                self.bump_preview.setPixmap(bump_pixmap)
                self.specular_preview.setPixmap(specular_pixmap)
                self.roughness_preview.setPixmap(roughness_pixmap)
                self.displacement_preview.setPixmap(displacement_pixmap)

                seed_info = f" (Seed: {self.seed_input.value()})" if self.seed_input.value() != -1 else ""
                self.update_status(
                    f"Base image saved: {base_path}{seed_info} | "
                ...
                history_entry = f"{timestamp} - {base_path} - Seed: {self.seed_input.value()}"
                self.history.append(history_entry)
                self.history_list.addItem(history_entry)

                # Create a Principled Shader and attach generated textures.
